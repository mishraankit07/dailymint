...
class LedgerEngineTest {
    ...
    @Test fun totalsAndReload() {
        val store = MemoryStore()
        val engine = LedgerEngine(store)
        assertTrue(engine.addEntry("1", "Salary", "1000", "Income", "2026-09-13", true).success)
        ...
        assertTrue(engine.addEntry("4", "Refund", "10", "Settlement", "2026-09-13", true).success)
        val totals = LedgerEngine(store).totals()
        assertEquals(100000L, totals.moneyIn)
        assertEquals(1000L, totals.neutralCredits)
        assertEquals(6288L, totals.spent)
        assertEquals(20000L, totals.invested)
        assertEquals(73712L, totals.remaining)
    ...
    @Test fun customTrackingCycleChangesMonthSummaryWindow() {
        ...
        assertTrue(engine.addEntry("3", "Salary", "100", "Income", "2026-09-01", true).success)
